...
export async function rdnsLookup(message: Message, target: string) {
    if (!target) {
        message.edit(fmtEmbed(message.content, createEmbed('rDNS - Usage', 'rdns <ip>', '#cdd6f4')));
        return;
    }
    ...
    try {
        ...
        if (hosts.length === 0) {
            message.edit(fmtEmbed(message.content, createEmbed('rDNS - Error', `No PTR records found for: ${target}`, '#f38ba8')));
            return;
        }
        ...
        message.edit(fmtEmbed(message.content, createEmbed('rDNS - Error', `Error performing reverse DNS: ${err.code || err.message}`, '#f38ba8')));
